...
**الطريقة المعتمدة: على نفس سيرفر الـ ERP** (Contabo VPS)، كمشروع منفصل تماماً
(مجلد + عملية Node.js عبر systemd + موقع Nginx خاص به). الدليل الكامل خطوة
بخطوة: [`DEPLOYMENT.md`](DEPLOYMENT.md). البناء يستخدم `output: "standalone"`
(مضبوط في `next.config.ts`) — حزمة تشغيل ذاتية الاكتفاء بدون الاعتماد على
منصّة مُدارة.

(لو حبيت لاحقاً استضافة على Vercel بدل ده، الموقع متوافق أصلاً — اضبط متغيّرات
`.env.example` في إعدادات المشروع هناك.)
